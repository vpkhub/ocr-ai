...
def validate_doc(gt_row, ocr_doc):
    ...
    ocr_fields = {f["field_name"].lower(): f for f in ocr_doc["extracted_fields"]}
    ...
    for field in gt_row.index:
        ...
        ocr_value = normalize(ocr_fields.get(field, {}).get("value", ""))

        match = gt_value == ocr_value
        confidence = ocr_fields.get(field, {}).get("confidence_score", None)

        results.append({
            "field_name": field,
            "ground_truth": gt_row[field],
            "ocr_value": ocr_fields.get(field, {}).get("value", ""),
...
def ocr_validation_ui():
    st.title("OCR Validation Dashboard (CSV/Excel + JSON)")
    ...
    if gt_file and ocr_file:
        ...
        ocr_dict = {doc["document_id"]: doc for doc in ocr_data}
        ...
        if not results_df.empty:
            ...
            doc_acc_df = results_df.groupby("document_id")["match"].mean().reset_index()
